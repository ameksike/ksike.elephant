...
*/
	public function scanDir($params) {
		...
		$data = kcl::package("dir")->scan($path, "-r",$this); // mode : [-i|-r]
	...
	public function searchDir($params) {
		echo "<pre>";
		$path = kcl::router("this")."dirs/";
		$data = kcl::package("dir")->find("d1-", $path, "", "-r" );  //type : [strict|sstrict|regular expresion]
		print_r($data);
		//return $data; 
	}
	...
	{
		echo "$path$item <br>";
		$a = array ("file"=> $lst["file"], "path"=>$path,"params"=>$params,"item"=>$item,"type"=>$typ3) ;
		print_r($a);
